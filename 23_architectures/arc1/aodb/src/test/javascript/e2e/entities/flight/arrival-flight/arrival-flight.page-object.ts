...
export class ArrivalFlightComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-arrival-flight div table .btn-danger'));
  title = element.all(by.css('jhi-arrival-flight div h2#page-heading span')).first();
...
export class ArrivalFlightUpdatePage {
  pageTitle = element(by.id('jhi-arrival-flight-heading'));
  ...
  async setClaimInput(claim: string): Promise<void> {
    await this.claimInput.sendKeys(claim);
  }

  async getClaimInput(): Promise<string> {
    return await this.claimInput.getAttribute('value');
  }
...
export class ArrivalFlightDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-arrivalFlight-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-arrivalFlight'));
